...
for t in range(T):
    ...
    if(ch==0):
        policy = Proj(policy,Vr,gradr/lambda_,t) ##define this
    else:
        count+=1
        policy = Proj(policy,Vc,gradc,1,t)
...
print(count)
with open("Store_robust_output_RS_latest_50","wb") as f:
    pickle.dump(store,f)
f.close()

with open("Policy_stored_robust_output_RS_latest_50","wb") as f:
    pickle.dump(store_pol,f)
f.close()
with open("Store_robust_vf_RS_latest_50","wb") as f:
    pickle.dump(vf_store,f)
f.close()

with open("Store_robust_cf_RS_latest_50","wb") as f:
